      ...
      <SRLWrapper>
        <ImageList variant="quilted" cols={4} rowHeight={200}>
          {itemData.map((item, index) => (
            ...
            >
              <Options />
              <img {...srcset(item.img, 200, pattern[index - Math.floor(index / pattern.length) * pattern.length].rows, pattern[index - Math.floor(index / pattern.length) * pattern.length].cols)} alt={item.title} loading="lazy" />
              <Typography
                variant="body2"
                component="span"
                sx={{
